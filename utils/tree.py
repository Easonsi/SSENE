...
def head_to_tree(head, dep_rel, len_=None, prune=-1, subj_pos=None, obj_pos=None, wp_conversion=True):
    if len_==None: len_ = len(head)
...
def tree_to_adj(sent_len, tree, directed=True, self_loop=False, identity_rel=constant.DEPREL_TO_ID['special_rel']):
    ...
    # BFS访问树结构, 根据节点的 children 列表和 dep_rel 属性填充 adj_matrix, dep_rel_matrix
    queue = [tree]
    idx = []
    while len(queue) > 0:
...
if __name__=="__main__":
    ...
    # adj_matrix, dep_rel_matrix = inputs_to_tree_reps(dep_head, dep_rel, maxlen=self.config.max_len)
    # seq_len = [len(i) for i in dep_head]
    dep_head = [6, 6, 6, 6, 6, 0, 13, 13, 13, 13, 13, 13, 6, 6, 6]
    # dep_rel = ['nsubj', 'dep', 'dep', 'neg', 'advmod', 'root', 'nsubj', 'nsubj', 'nsubj', 'nsubj', 'nsubj', 'advmod', 'conj', 'conj', 'punct']
    dep_rel = list(range(15))
    tree = head_to_tree(dep_head, dep_rel)
    adj_matrix, dep_rel_matrix = tree_to_adj(
        15, tree, directed=False, self_loop=True)
    print(adj_matrix)
